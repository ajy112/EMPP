...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for step, data in enumerate(data_loader):
        ...
        with amp_autocast():
            target_processed = (data.y[:, target] - task_mean) / task_std

            if ssp:
                pred, EMPP_loss = model(pos=data.pos, batch=data.batch, 
                node_atom=data.z, ptr=data.ptr, target=target_processed, ssp=True)
                pred = pred.squeeze()
                loss = criterion(pred, target_processed)
                loss += EMPP_loss
            else:
                pred = model(pos=data.pos, batch=data.batch, 
                node_atom=data.z, ptr=data.ptr, target=None, ssp=False)
                pred = pred.squeeze()
                loss = criterion(pred, target_processed)
